...
- **B. One `builds/` folder** at the workspace root, one sub-folder per tier-2 and tier-3 build, each with the `BUILD-STATUS.md` template, plus `builds/STATUS.md` as the catch-all. Tier 1 is unchanged by this option: it uses a scratch folder and leaves only its `runs.md` row. *Trade-off:* a folder per medium build accumulates; the user has to prune or accept them.
...
1. **Install the protocol** per DP-6: copy `skill/` (`SKILL.md` + `references/prompts.md` + `references/research.md`) to the chosen location. Then adapt minimally, and record every edit in `STATUS.md`:
   ...
   - DP-5: under B or C, replace the percentage ladder sentence in `SKILL.md` § 3 with this one, so a rule still stands: "Retire at every hand-off point the plan names, and never run more than two steps past step 5 in one chat."
...
5. **First feedback-loop row** (DP-4 A or B; under C there is no runs log, so skip this step and say so). Append the run to `runs.md`: what the build was, what the user corrected, what got in the way of the locked goal, whether the research was too much or too little. If a rule in `SKILL.md` got in the way, edit it now.
