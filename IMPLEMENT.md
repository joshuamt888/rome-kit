...
1. **What am I?** Platform and capabilities: can I create files? run shell commands? spawn a sub-agent with no inherited context? persist memory across sessions? read my own context-window usage? Is there a second model I can call from the shell (a different vendor's coding CLI, read-only)?
...
4. **Existing skills.** Does the platform discover skills from a directory it auto-loads, or must the protocol be referenced from a rules file, or pasted? Do they already have a skill that turns a plan into a skill, builds knowledge packs, or exports kits? Rome calls those as-is at step 6, so note their names.
...
**Options:**
- **A. A different model, run read-only from the shell.** A second vendor's coding CLI reads the build folder and returns a `VERDICT:` line. *Trade-off:* needs a shell and a second subscription or API; the command in `skill/references/prompts.md` has to be adapted to that CLI's flags.
...
- **C. A fresh chat window the user shuttles.** The AI writes the prompt to a file, the user pastes it into a new chat with the build folder attached, and brings the verdict back. *Trade-off:* two manual hops per check; otherwise as good as B.
...
**Options:**
- **A. Named sub-agents with fixed model and effort:** a `researcher` (mid-cost, medium effort) for tier-3 research fan-out and transcript digests, a `builder` (mid-cost, medium) for one file from an approved plan and for one fixture cold-run at step 7, a `grep-reader` (mid-cost, low) for inventories and bulk parsing, a `checker` (strongest model, high) for red-teams and evidence when DP-2 chose B. Never an unnamed agent that inherits the session's effort. *Trade-off:* the definitions have to be written once; the platform must support per-agent model and effort settings.
...
1. **Install the protocol** per DP-6: copy `skill/` (`SKILL.md` + `references/prompts.md` + `references/research.md`) to the chosen location. Then adapt minimally, and record every edit in `STATUS.md`:
   - DP-2: in `references/prompts.md`, replace the command block under "second-reviewer command" with the real command for the chosen reviewer (or, under C, a one-line note: "write the prompt to `checks/<step>-prompt.md`; the user pastes it into a fresh chat").
   - DP-5: under B or C, delete the percentage ladder sentence in `SKILL.md` § 3 and keep "retire at the plan's hand-off points".
   - DP-1: replace "the catch-all status file" in `SKILL.md` § 1 with its real path.
   - Rename only terms that clash with the user's vocabulary.
2. **Create the agent definitions** (DP-3 A only). One file per agent in the platform's agents folder: name, one-line description, model, effort, and a five-line body ("do only the assigned task; write exactly one file at the path the caller names; do not launch agents; do not add scope"). Four agents: `researcher`, `builder`, `grep-reader`, `checker`. Skip under B and C.
3. **Install the templates** so they outlive this kit folder: copy `templates/BUILD-STATUS.md` and `templates/NEXT-CHAT.md` to `<skill install path>/templates/`. Then **create the runs log** (DP-4 A or B): copy `templates/runs.md` to `<skill install path>/references/runs.md`, empty rows only.
...
5. **Wire the standing-instructions file.** This is the stickiness step: an install that only creates folders dies at the user's first new chat. Merge into their `CLAUDE.md` / `AGENTS.md` / rules file, in their format:
   ...
   **Scope warning:** some platforms scope instruction files to a workspace rather than the whole account (Cursor: `.cursor/rules/*.mdc` with `alwaysApply: true`, or the rule may never fire). Put the wiring where the user's AI actually works and **tell them which file you edited.**
...
4. **Verification test 2, the cold read.** In a fresh session, hand over only the build's STATUS (tier 2 or 3) or the runs row (tier 1) and ask: "What step of 8 is this build at, what is the locked goal, and what would the next chat do first?" A correct answer from the file alone proves the paper works as memory. Record both tests' sentences and outcomes in `STATUS.md`.
