...
| `agents/*.md` | The platform's agents folder, one file each (`researcher`, `builder`, `grep-reader`, `checker`) | DP-3 A only | Replace the model line with the platform's real model name for that class. Skipped on single-thread and chat-only installs. |
